...
# FastAPI setup (define FIRST so you can use @app.post later)
app = FastAPI()
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# --- Load shift model ---
shift_model = torch.load("mlp_shift_model.pth", map_location=device)
shift_model.eval()
shift_scaler = joblib.load("mlp_shift_scaler.pkl")

# --- Pydantic model for shift prediction ---
...
def predict_shift(data: ShiftDataRequest):
    shift_map = {"Afternoon": [1, 0], "Night": [0, 1], "Morning": [0, 0]}
    exp_map = {"Beginner": [1, 0, 0], "Experienced": [0, 1, 0], "Expert": [0, 0, 1], "Intermediate": [0, 0, 0]}
    ...
    ] + shift_map.get(data.shift_type, [0, 0]) \
      + exp_map.get(data.experience_level, [0, 0, 0]) \
...
def append_shift_entry(data: ShiftDataRequest):
    ...
    # Risk label
    if predicted_time_cycles < 60:
        risk = "High"
    elif predicted_time_cycles < 120:
        risk = "Medium"
    else:
        risk = "Low"
